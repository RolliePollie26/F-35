...
#ifdef F35A_FM_EXPORTS
#define F35A_FM_API __declspec(dllexport)
#else
#define F35A_FM_API __declspec(dllimport)
#endif
...
extern "C" 
{
	...
	body coordinate system system is always X - positive forward ,
											...
											Z - positive to right 
	*/
	F35A_FM_API void ed_fm_add_local_force(double & x,double &y,double &z,double & pos_x,double & pos_y,double & pos_z);
	F35A_FM_API void ed_fm_add_global_force(double & x,double &y,double &z,double & pos_x,double & pos_y,double & pos_z);
	...
	*/
	F35A_FM_API bool ed_fm_add_local_force_component (double & x,double &y,double &z,double & pos_x,double & pos_y,double & pos_z);
	F35A_FM_API bool ed_fm_add_global_force_component (double & x,double &y,double &z,double & pos_x,double & pos_y,double & pos_z);

	F35A_FM_API void ed_fm_add_local_moment(double & x,double &y,double &z);
	F35A_FM_API void ed_fm_add_global_moment(double & x,double &y,double &z);
	...
	*/
	F35A_FM_API bool ed_fm_add_local_moment_component (double & x,double &y,double &z);
	F35A_FM_API bool ed_fm_add_global_moment_component (double & x,double &y,double &z);
	...
	*/
	F35A_FM_API void ed_fm_simulate(double dt);

	/*
	called before simulation to set up your environment for the next step
	give parameters of surface under your aircraft usefull for ground effect
	*/
	F35A_FM_API void ed_fm_set_surface (double		h,//surface height under the center of aircraft
	...
	called before simulation to set up your environment for the next step
	*/
	F35A_FM_API void ed_fm_set_atmosphere(double h,//altitude above sea level
	...
	called before simulation to set up your environment for the next step
	*/
	F35A_FM_API void ed_fm_set_current_mass_state (double mass,
	...
	called before simulation to set up your environment for the next step
	*/
	F35A_FM_API void ed_fm_set_current_state (double ax,//linear acceleration component in world coordinate system
	...
	*/
	F35A_FM_API void ed_fm_set_current_state_body_axis(double ax,//linear acceleration component in body coordinate system
	...
	input handling
	*/
	F35A_FM_API void ed_fm_set_command (int command,
							 float value);
	/*
	 Mass handling 
	...
	*/
	F35A_FM_API bool ed_fm_change_mass  (double & delta_mass,
	...
	*/
	F35A_FM_API void ed_fm_set_internal_fuel(double fuel);

	/*
		get internal fuel volume 
	*/
	F35A_FM_API double ed_fm_get_internal_fuel();

	/*
		set external fuel volume for each payload station , called for weapon init and on reload
	*/
	F35A_FM_API void ed_fm_set_external_fuel(int	 station,
														double fuel,
														double x,
														double y,
														double z);
	...
	*/
	F35A_FM_API double ed_fm_get_external_fuel();
	...
	*/
	F35A_FM_API void ed_fm_refueling_add_fuel(double fuel);
	...
	*/
	F35A_FM_API void ed_fm_set_draw_args_v2 (float * drawargs,size_t size);
	F35A_FM_API void ed_fm_set_fc3_cockpit_draw_args_v2 (float * drawargs,size_t size);

	/*
	shake level amplitude for head simulation , 
	*/
	F35A_FM_API double ed_fm_get_shake_amplitude();  

	/*
	will be called for your internal configuration
	*/
	F35A_FM_API void ed_fm_configure(const char * cfg_path);

	/*
	will be called for your internal configuration
	void ed_fm_release   called when fm not needed anymore : aircraft death etc.
	*/
	F35A_FM_API void ed_fm_release ();

	/*
	various param call back to proper integrate your FM to DCS , like engine RPM , thrust etc
	*/
	F35A_FM_API double ed_fm_get_param(unsigned param_enum);
	...
	*/
	F35A_FM_API void ed_fm_cold_start();
	F35A_FM_API void ed_fm_hot_start();
	F35A_FM_API void ed_fm_hot_start_in_air();
	...
	*/
	F35A_FM_API bool ed_fm_make_balance (double & ax,//linear acceleration component in world coordinate system);
									  double & ay,//linear acceleration component in world coordinate system
									  double & az,//linear acceleration component in world coordinate system
									  double & vx,//linear velocity component in world coordinate system
									  double & vy,//linear velocity component in world coordinate system
	...
	*/
	F35A_FM_API bool ed_fm_enable_debug_info();
	...
	F35A_FM_API size_t ed_fm_debug_watch(int level, char *buffer,size_t maxlen);

	/* 
	path to your plugin installed
	*/
	F35A_FM_API void ed_fm_set_plugin_data_install_path(const char * path);

	// damages and failures
	// callback when preplaneed failure triggered from mission 
	F35A_FM_API void ed_fm_on_planned_failure(const char * data);

	// callback when damage occurs for airframe element 
	F35A_FM_API void ed_fm_on_damage(int Element, double element_integrity_factor); 

	// called in case of repair routine 
	F35A_FM_API void ed_fm_repair(); 

	// in case of some internal damages or system failures this function return true , to switch on repair process
	F35A_FM_API bool ed_fm_need_to_be_repaired(); 

	// inform about  invulnerability settings
	F35A_FM_API void ed_fm_set_immortal(bool value); 
	// inform about  unlimited fuel
	F35A_FM_API void ed_fm_unlimited_fuel(bool value); 
	// inform about simplified flight model request 
	F35A_FM_API void ed_fm_set_easy_flight(bool value); 

	// custom properties sheet 
	F35A_FM_API void ed_fm_set_property_numeric(const char * property_name,float value); 
	// custom properties sheet 
	F35A_FM_API void ed_fm_set_property_string(const char * property_name,const char * value); 
	...
	*/
	F35A_FM_API bool ed_fm_pop_simulation_event(ed_fm_simulation_event & out);

	/*
		feedback to your fm about suspension state
	*/
	F35A_FM_API void ed_fm_suspension_feedback(int idx,const ed_fm_suspension_info * info);
};

#endif // ifndef _F35A_H_
